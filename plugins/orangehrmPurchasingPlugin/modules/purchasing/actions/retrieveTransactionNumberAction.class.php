 ...
 */
class retrieveTransactionNumberAction extends sfAction {
    ...
    public function getRequestSubmissionService() {
        ...
        public function execute($request) {
            ...
            $emp = $request->getParameter('user');
            $date = $request->getParameter('date');
            $reqMaster = $this->getRequestSubmissionService()->getRequestByMaster($date);
            
            $num= 01;
            foreach ($reqMaster as $rM) {
                $num += 1;
            }
            return $this->renderText($prod."/".$emp."/".$date."/".$num);
        }
}
