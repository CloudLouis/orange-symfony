<?php
// Connection Component Binding
Doctrine_Manager::getInstance()->bindComponent('OhrmUser', 'doctrine');

/**
 * BaseOhrmUser
 * 
 * This class has been auto-generated by the Doctrine ORM Framework
 * 
 * @property integer $id
 * @property integer $user_role_id
 * @property integer $emp_number
 * @property string $user_name
 * @property string $user_password
 * @property integer $deleted
 * @property integer $status
 * @property timestamp $date_entered
 * @property timestamp $date_modified
 * @property integer $modified_user_id
 * @property integer $created_by
 * @property HsHrEmployee $HsHrEmployee
 * @property OhrmUserRole $OhrmUserRole
 * @property Doctrine_Collection $HsHrMailnotifications
 * @property Doctrine_Collection $OhrmLeaveAdjustment
 * @property Doctrine_Collection $OhrmLeaveComment
 * @property Doctrine_Collection $OhrmLeaveEntitlement
 * @property Doctrine_Collection $OhrmLeaveRequestComment
 * @property Doctrine_Collection $OhrmOpenidUserIdentity
 * @property Doctrine_Collection $OhrmPerformanceTrackerLog
 * @property Doctrine_Collection $OhrmQuotations
 * @property Doctrine_Collection $OhrmQuotations_3
 * @property Doctrine_Collection $OhrmQuotations_4
 * @property Doctrine_Collection $OhrmTimesheetActionLog
 * @property Doctrine_Collection $Quotations
 * 
 * @method integer             getId()                        Returns the current record's "id" value
 * @method integer             getUserRoleId()                Returns the current record's "user_role_id" value
 * @method integer             getEmpNumber()                 Returns the current record's "emp_number" value
 * @method string              getUserName()                  Returns the current record's "user_name" value
 * @method string              getUserPassword()              Returns the current record's "user_password" value
 * @method integer             getDeleted()                   Returns the current record's "deleted" value
 * @method integer             getStatus()                    Returns the current record's "status" value
 * @method timestamp           getDateEntered()               Returns the current record's "date_entered" value
 * @method timestamp           getDateModified()              Returns the current record's "date_modified" value
 * @method integer             getModifiedUserId()            Returns the current record's "modified_user_id" value
 * @method integer             getCreatedBy()                 Returns the current record's "created_by" value
 * @method HsHrEmployee        getHsHrEmployee()              Returns the current record's "HsHrEmployee" value
 * @method OhrmUserRole        getOhrmUserRole()              Returns the current record's "OhrmUserRole" value
 * @method Doctrine_Collection getHsHrMailnotifications()     Returns the current record's "HsHrMailnotifications" collection
 * @method Doctrine_Collection getOhrmLeaveAdjustment()       Returns the current record's "OhrmLeaveAdjustment" collection
 * @method Doctrine_Collection getOhrmLeaveComment()          Returns the current record's "OhrmLeaveComment" collection
 * @method Doctrine_Collection getOhrmLeaveEntitlement()      Returns the current record's "OhrmLeaveEntitlement" collection
 * @method Doctrine_Collection getOhrmLeaveRequestComment()   Returns the current record's "OhrmLeaveRequestComment" collection
 * @method Doctrine_Collection getOhrmOpenidUserIdentity()    Returns the current record's "OhrmOpenidUserIdentity" collection
 * @method Doctrine_Collection getOhrmPerformanceTrackerLog() Returns the current record's "OhrmPerformanceTrackerLog" collection
 * @method Doctrine_Collection getOhrmQuotations()            Returns the current record's "OhrmQuotations" collection
 * @method Doctrine_Collection getOhrmQuotations3()           Returns the current record's "OhrmQuotations_3" collection
 * @method Doctrine_Collection getOhrmQuotations4()           Returns the current record's "OhrmQuotations_4" collection
 * @method Doctrine_Collection getOhrmTimesheetActionLog()    Returns the current record's "OhrmTimesheetActionLog" collection
 * @method Doctrine_Collection getQuotations()                Returns the current record's "Quotations" collection
 * @method OhrmUser            setId()                        Sets the current record's "id" value
 * @method OhrmUser            setUserRoleId()                Sets the current record's "user_role_id" value
 * @method OhrmUser            setEmpNumber()                 Sets the current record's "emp_number" value
 * @method OhrmUser            setUserName()                  Sets the current record's "user_name" value
 * @method OhrmUser            setUserPassword()              Sets the current record's "user_password" value
 * @method OhrmUser            setDeleted()                   Sets the current record's "deleted" value
 * @method OhrmUser            setStatus()                    Sets the current record's "status" value
 * @method OhrmUser            setDateEntered()               Sets the current record's "date_entered" value
 * @method OhrmUser            setDateModified()              Sets the current record's "date_modified" value
 * @method OhrmUser            setModifiedUserId()            Sets the current record's "modified_user_id" value
 * @method OhrmUser            setCreatedBy()                 Sets the current record's "created_by" value
 * @method OhrmUser            setHsHrEmployee()              Sets the current record's "HsHrEmployee" value
 * @method OhrmUser            setOhrmUserRole()              Sets the current record's "OhrmUserRole" value
 * @method OhrmUser            setHsHrMailnotifications()     Sets the current record's "HsHrMailnotifications" collection
 * @method OhrmUser            setOhrmLeaveAdjustment()       Sets the current record's "OhrmLeaveAdjustment" collection
 * @method OhrmUser            setOhrmLeaveComment()          Sets the current record's "OhrmLeaveComment" collection
 * @method OhrmUser            setOhrmLeaveEntitlement()      Sets the current record's "OhrmLeaveEntitlement" collection
 * @method OhrmUser            setOhrmLeaveRequestComment()   Sets the current record's "OhrmLeaveRequestComment" collection
 * @method OhrmUser            setOhrmOpenidUserIdentity()    Sets the current record's "OhrmOpenidUserIdentity" collection
 * @method OhrmUser            setOhrmPerformanceTrackerLog() Sets the current record's "OhrmPerformanceTrackerLog" collection
 * @method OhrmUser            setOhrmQuotations()            Sets the current record's "OhrmQuotations" collection
 * @method OhrmUser            setOhrmQuotations3()           Sets the current record's "OhrmQuotations_3" collection
 * @method OhrmUser            setOhrmQuotations4()           Sets the current record's "OhrmQuotations_4" collection
 * @method OhrmUser            setOhrmTimesheetActionLog()    Sets the current record's "OhrmTimesheetActionLog" collection
 * @method OhrmUser            setQuotations()                Sets the current record's "Quotations" collection
 * 
 * @package    orangehrm
 * @subpackage model\base
 * @author     Your name here
 * @version    SVN: $Id: Builder.php 7490 2010-03-29 19:53:27Z jwage $
 */
abstract class BaseOhrmUser extends sfDoctrineRecord
{
    public function setTableDefinition()
    {
        $this->setTableName('ohrm_user');
        $this->hasColumn('id', 'integer', 4, array(
             'type' => 'integer',
             'fixed' => 0,
             'unsigned' => false,
             'primary' => true,
             'autoincrement' => true,
             'length' => 4,
             ));
        $this->hasColumn('user_role_id', 'integer', 4, array(
             'type' => 'integer',
             'fixed' => 0,
             'unsigned' => false,
             'primary' => false,
             'notnull' => true,
             'autoincrement' => false,
             'length' => 4,
             ));
        $this->hasColumn('emp_number', 'integer', 4, array(
             'type' => 'integer',
             'fixed' => 0,
             'unsigned' => false,
             'primary' => false,
             'notnull' => false,
             'autoincrement' => false,
             'length' => 4,
             ));
        $this->hasColumn('user_name', 'string', 40, array(
             'type' => 'string',
             'fixed' => 0,
             'unsigned' => false,
             'primary' => false,
             'notnull' => false,
             'autoincrement' => false,
             'length' => 40,
             ));
        $this->hasColumn('user_password', 'string', 255, array(
             'type' => 'string',
             'fixed' => 0,
             'unsigned' => false,
             'primary' => false,
             'notnull' => false,
             'autoincrement' => false,
             'length' => 255,
             ));
        $this->hasColumn('deleted', 'integer', 1, array(
             'type' => 'integer',
             'fixed' => 0,
             'unsigned' => false,
             'primary' => false,
             'default' => '0',
             'notnull' => true,
             'autoincrement' => false,
             'length' => 1,
             ));
        $this->hasColumn('status', 'integer', 1, array(
             'type' => 'integer',
             'fixed' => 0,
             'unsigned' => false,
             'primary' => false,
             'default' => '1',
             'notnull' => true,
             'autoincrement' => false,
             'length' => 1,
             ));
        $this->hasColumn('date_entered', 'timestamp', 25, array(
             'type' => 'timestamp',
             'fixed' => 0,
             'unsigned' => false,
             'primary' => false,
             'notnull' => false,
             'autoincrement' => false,
             'length' => 25,
             ));
        $this->hasColumn('date_modified', 'timestamp', 25, array(
             'type' => 'timestamp',
             'fixed' => 0,
             'unsigned' => false,
             'primary' => false,
             'notnull' => false,
             'autoincrement' => false,
             'length' => 25,
             ));
        $this->hasColumn('modified_user_id', 'integer', 4, array(
             'type' => 'integer',
             'fixed' => 0,
             'unsigned' => false,
             'primary' => false,
             'notnull' => false,
             'autoincrement' => false,
             'length' => 4,
             ));
        $this->hasColumn('created_by', 'integer', 4, array(
             'type' => 'integer',
             'fixed' => 0,
             'unsigned' => false,
             'primary' => false,
             'notnull' => false,
             'autoincrement' => false,
             'length' => 4,
             ));
    }

    public function setUp()
    {
        parent::setUp();
        $this->hasOne('HsHrEmployee', array(
             'local' => 'emp_number',
             'foreign' => 'emp_number'));

        $this->hasOne('OhrmUserRole', array(
             'local' => 'user_role_id',
             'foreign' => 'id'));

        $this->hasMany('HsHrMailnotifications', array(
             'local' => 'id',
             'foreign' => 'user_id'));

        $this->hasMany('OhrmLeaveAdjustment', array(
             'local' => 'id',
             'foreign' => 'created_by_id'));

        $this->hasMany('OhrmLeaveComment', array(
             'local' => 'id',
             'foreign' => 'created_by_id'));

        $this->hasMany('OhrmLeaveEntitlement', array(
             'local' => 'id',
             'foreign' => 'created_by_id'));

        $this->hasMany('OhrmLeaveRequestComment', array(
             'local' => 'id',
             'foreign' => 'created_by_id'));

        $this->hasMany('OhrmOpenidUserIdentity', array(
             'local' => 'id',
             'foreign' => 'user_id'));

        $this->hasMany('OhrmPerformanceTrackerLog', array(
             'local' => 'id',
             'foreign' => 'user_id'));

        $this->hasMany('OhrmQuotations', array(
             'local' => 'id',
             'foreign' => 'approved_by'));

        $this->hasMany('OhrmQuotations as OhrmQuotations_3', array(
             'local' => 'id',
             'foreign' => 'recipient_id'));

        $this->hasMany('OhrmQuotations as OhrmQuotations_4', array(
             'local' => 'id',
             'foreign' => 'requester_id'));

        $this->hasMany('OhrmTimesheetActionLog', array(
             'local' => 'id',
             'foreign' => 'performed_by'));

        $this->hasMany('Quotations', array(
             'local' => 'id',
             'foreign' => 'recipient_id'));
    }
}